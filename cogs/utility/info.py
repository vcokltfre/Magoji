...
class AllInfo(commands.Cog):
    ...
    async def _guild(self, ctx):
        ...
        embed.add_field(
            name=f"**Total Member Count:** {len(members)}",
        ...
        embed.add_field(
            ...
            value="\n".join(
                f"{category.name}: {len(category.channels)}"
        ...
        embed.set_footer(
            text=f"Command Invoked by {ctx.author}", icon_url=ctx.author.avatar_url
        )
    ...
    async def _user(self, ctx, user: Union[discord.Member, discord.User] = None):
        ...
        embed = discord.Embed(title=f"{str(user).title()}'s Info", colour=0x87CEEB)
    ...
    @commands.group(aliases=("src", "github", "git"), invoke_without_command=True)
    async def source(
